...
-- Test 22: Error path - INCLUDE with 2-key index (line 706)
DO $$
BEGIN
    CREATE UNLOGGED TABLE t_err_2key_inc(k1 int4, k2 int4, v int4);
    BEGIN
        CREATE INDEX t_err_2key_inc_smol ON t_err_2key_inc USING smol(k1, k2) INCLUDE (v);
        RAISE EXCEPTION 'Expected error for 2-key + INCLUDE was not raised';
    EXCEPTION
        WHEN OTHERS THEN
            IF SQLERRM LIKE '%INCLUDE columns currently supported only for single-key%' THEN
                RAISE NOTICE 'Correctly caught 2-key+INCLUDE error';
            ELSE
                RAISE;
            END IF;
    END;
    DROP TABLE t_err_2key_inc CASCADE;
END $$;
